...
export default function NudgeSuggestions({ currentMood, quietHoursEnabled }: NudgeSuggestionsProps) {
  ...
  const generateSuggestions = useCallback(() => {
    const allSuggestions: Suggestion[] = [
      ...
      // Productivity suggestions
      {
        ...
        description: 'Review your to-do list and focus on the most important items',
        icon: '📋',
    ...
    if (currentMood !== null) {
      if (currentMood <= 2) {
        ...
        filteredSuggestions = allSuggestions.filter(s => 
          s.category === 'wellness' || s.category === 'break'
        );
      } else if (currentMood >= 4) {
        // High mood - can focus on productivity
        filteredSuggestions = allSuggestions.filter(s => 
          s.category === 'productivity' || s.category === 'focus'
        );
      }
    }
    ...
    if (quietHoursEnabled) {
      filteredSuggestions = filteredSuggestions.filter(s => 
        s.category !== 'break' || s.id === 'take-break'
      );
    }
